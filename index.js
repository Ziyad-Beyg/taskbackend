...
app.get('/get-all-books', async (req, res) => {
    console.log(req.url);
    let allBooks = await Book.find(); // returns array
    res.json(allBooks);
});
...
app.get('/get-one-book/:bookName', async (req, res) => {
    ...
    let allBooks = await Book.findOne({ name: bookName }); // returns one object
    res.json(allBooks);
});
...
app.post('/update-book/:id', async (req, res) => {
    ...
    let updated = await Book.findOneAndUpdate({ _id: id },
